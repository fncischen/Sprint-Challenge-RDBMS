...
RDBMS is an API / interface we can use that mimics SQL, whereas SQL, search query language, is the foundation script / code to manipulate large data sets.

1. Why do tables need a `primary key`?

The priamry key acts as a access key that allows the client to make requests to the server. 
...
We need to establish a join that allows us to merge one foriegn key with another foriegn key.
